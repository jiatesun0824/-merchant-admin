                    ...
                    <el-form-item label="价格设置" required>
                        <el-form :model="item" label-width="0" class="demo-ruleForm lines magin30" size="small" v-for="(item,index) in ruleForm.prices">
                            <el-row :gutter="5">
                                <el-col :md="2" :lg="3" >
                                    <el-form-item>
                                        <el-input-number @change="priceFun(index)" :precision="0" :min="0" :max="999999" v-model="item.price" placeholder="套餐价格"></el-input-number >
                                ...
                                <el-col :md="2" :lg="3" >
                                    <el-form-item>
                                        <el-input-number :min="0" :precision="0"  :max="maxType"  @change="durationFun(index)"  placeholder="套餐时长" v-model="item.duration"></el-input-number >
...
export default {
  ...
  methods: {
    ...
    submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
            if(valid&&!flags){
                ...
                this.API.addMeal(ruleForm).then(res=>{
                    if(res.success){
                        this.$message.success('套餐添加成功！');
                        setTimeout(res=>{ this.$router.push({path: '/packagelist/list'}); },500)
                    }else{
    ...
    empty(){
        this.ruleForm = {
          servicesName: '', serviceDesc:'', userScope:'', servicesType: '', saleChannel:[], remark: '', creator: sessionStorage.getItem('userId'),
          prices:[{"duration": '', "freeRenderDuration": '',"giveDuration": '',"price": '',"priceUnit": "0", "sanduCurrency": '' }],
        }
    },
    addMore(){
          ...
          this.ruleForm.prices.push({"duration": '',"freeRenderDuration": '',"giveDuration": '',"price": '',"priceUnit": "0","sanduCurrency": ''});
    },
    delMore(index){

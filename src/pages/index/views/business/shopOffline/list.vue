    ...
    <div class="shopOffline">
        <div class="shopOffline-header">
            ...
            <div class="header-screen">
                <el-input class="header-screen-input" size="medium" placeholder="请输入店铺名称" v-model="shopName"
                          @change="listShopOffline('店铺名称')"></el-input>
                <el-select class="screen-city-item" clearable v-model="provinceCode" size="medium" placeholder="请选择省"
                           @change="listShopOffline('省')">
                    <el-option
                ...
                <el-select class="screen-city-item" clearable v-model="cityCode" size="medium" placeholder="请选择市"
                           @change="listShopOffline('市')">
                    <el-option
                            v-for="item in city"
                            :key="item.areaCode"
                            :label="item.areaName"
                ...
                <el-select class="screen-city-item" clearable v-model="districtCode" size="medium" placeholder="请选择区"
                           @change="listShopOffline('区')">
                    <el-option
                ...
                <el-select class="screen-city-item" clearable v-model="streetCode" size="medium" placeholder="请选择街道"
                           @change="listShopOffline('街道')">
                    <el-option
            ...
            <div class="header-seek">
                <el-button type="primary" round class="pri UnifiedsearchBtn" size="medium" @click="doQuery">搜索
                </el-button>
                <el-button round class="seek UnifiedsearchBtn" size="medium" @click="listShopOffline('重置')">重置
                </el-button>
            </div>
        </div>
        <div class="shopOffline-content">
            <el-table
                    ...
                    :data="shopData"
                    style="width: 100%">
                <el-table-column prop="logoPicPath" label="LOGO" class="t-c">
                    <template slot-scope="scope">
                        <img v-if="scope.row.logoPicPath" :alt="scope.row.logoPicPath" :src="scope.row.logoPicPath" width="50" height="50"
                             @click="goClaimShopView(scope.row)">
                        <div v-else class="headPicPath" width="50" height="50">暂无图片</div>
                    </template>
                </el-table-column>
                <el-table-column prop="shopName" label="店铺名称">
                    <template slot-scope="scope">
                        <span class="shop-name" @click="goClaimShopView(scope.row)">{{scope.row.shopName}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="contactName" label="联系人"></el-table-column>
                <el-table-column prop="contactPhone" label="联系人电话"></el-table-column>
                <el-table-column prop="longAreaName" label="区域">
                     <template slot-scope="scope">
                        <el-tooltip
                                effect="dark"
                ...
                <el-table-column label="操作">
                    <template slot-scope="scope">
                        <div>
                            <span class="operation" @click="goClaimShopView(scope.row)"
                                  v-if="isPer('shopOffline:dealer:receive')">认领</span>
                        </div>
                    </template>
                </el-table-column>
            </el-table>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            /** 认领 */
            goClaimShopView(row) {
                this.$router.push(`/shopOffline/shopOfflineDetail/${row.id}`);
            },
            /*省市区联动*/

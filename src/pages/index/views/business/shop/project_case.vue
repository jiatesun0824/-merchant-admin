    ...
    <div class="project-case-center">
      ...
      <div class="project-case-list">
        ...
        <div class="item" v-for="(item, index) in projectCaseList">
          ...
          <img v-if="item.coverPicPath" :src="BASE_URL.sourceBaseUrl + item.coverPicPath" alt>
          ...
          <div class="operation">
            ...
            <span
              @click="projectCasePublish(index, $event)"
            >{{item.releaseStatus == 1? '取消发布' : '发布'}}</span>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    goAddProjectCase(type, id) {
      ...
      this.$router.push({
        ...
        query: { shopType: this.shopType }
      });
    },
    /*发布，取消发布*/
    projectCasePublish(index, event) {
      this.$confirm(
        "此操作将" + event.target.innerText + "当前店铺信息，是否继续",
        "提示",
        {
        ...
        .then(() => {
          this.API.projectCasePublish({
            ...
            if (res.success) {
              ...
              this.projectCaseList[index].releaseStatus =
                event.target.innerText == "发布" ? 1 : 0;
            } else {
              this.$message({
                type: "error",
                message: res.message

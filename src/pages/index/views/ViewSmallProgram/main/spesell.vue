...
<!-- 特卖页面-->
    <div class="spesell">
        <el-main>
            <!-- 添加 按钮 -->
            <div class="btnbox">
                <span class="addbtn UnifiedsearchBtn" @click="SpesellList">添加</span>
            </div>
        ...
        <!-- 添加特卖 弹框 -->
        <el-dialog  :visible.sync="addspesell" height="700px" width="990px" title="添加特卖商品">
           <div class="diabox">
           <div class="header">
                    <span>
                        编号
            ...
            <ul class="sellbox">
                 <div class="golistbox" v-if="isShowNone"
                 ...
                 >
                    <div class="nonetextbox">
                        <h1 class="noneshop">暂无新品，请在商品管理页面设置特卖商品</h1>
                        <p class="shoptext">设置方式: 在 <span class="golist" @click="$router.push('/goods')">商品管理列表</span> 中选择商品 → 编辑 → 勾选特卖</p>
                    </div>
                </div>
                <li class="list" 
                v-for="(item,index) in spesellList" :key="index"
                    ...
                    <span class="name"><!--商品名字-->
                      {{item.spuName}}
                    </span>
                    <span><!--商品分类-->
                       {{item.bigType}}>{{item.smallType}}
                    </span>
                    <span v-if="item.maxSalePrice==item.minSalePrice"><!--原价-->
                    ...
                    <span v-if="item.maxPrice == item.minPrice "><!--优惠价-->
                           {{item.minPrice}}
                    </span>
                    <span v-else>
   ...
   SpesellmMainList(){
      this.API.sepsellmainlist({
          ...
          if(this.spesell_mainList[i].picPath){
            this.spesell_mainList[i].picPath = process.env.sourceBaseUrl + this.spesell_mainList[i].picPath;
          }
          }
      })
   },
    ...
    SpesellList(){
      ...
      for (let i = 0; i < this.spesellList.length; i++) {
          if(this.spesellList[i].picPath){
            this.spesellList[i].picPath = process.env.sourceBaseUrl + this.spesellList[i].picPath;
          }
          }
        })
    }

        ...
        <div class="addWheelAct-content">
            <el-form ref="form" :model="formData" label-width="80px" :rule="rules">
                ...
                <el-form-item label="活动时间">
                    <el-date-picker v-model="formData.beginTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" :disabled="true"
                                    placeholder="请设置起始时间" align="right" :picker-options="pickerOptionsStart"></el-date-picker>
                    -- 至 --
                    <el-date-picker v-model="formData.endTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" :disabled="true"
                ...
                <el-form-item>
                    <el-button type="primary" @click="cretedFunc">修改</el-button>
    ...
    export default {
        ...
        created(){
            this.API.luckywheelDetail({actId:this.$route.query.id}).then(res=>{
                if(res.success){
                   this.formData.actName=res.obj.name;
                   this.formData.beginTime=res.obj.beginTime;
                   this.formData.endTime=res.obj.endTime;
                   this.formData.actRule=res.obj.rule;
                   for (let key in this.formData) {
                       this.formData[key]=res.obj[key];
                   }
                   this.configItem=res.obj.configItem.toString();
                }else {
        ...
        methods:{
            cretedFunc(){
                ...
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        this.API.luckywheelModify(data).then(res=>{
                            if(res.success){
